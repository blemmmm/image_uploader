...
const spawn = (command, args) => {
  ...
  const spawned_process = child_process.spawn(command, args, {
    stdio: "inherit",
  });
 ...
 */
const process_close_event = (process) =>
...
process.nextTick(async () => {
  try {
    ...
    assert(
      fs.existsSync(esbuild_entry_path) === true,
      "Invalid ESBuild entry."
    );

    // PostCSS entry
    const postcss_entry_path = path.join(cwd, "./client/src/postcss.css");
    assert(
      fs.existsSync(postcss_entry_path) === true,
      "Invalid PostCSS entry."
    );
